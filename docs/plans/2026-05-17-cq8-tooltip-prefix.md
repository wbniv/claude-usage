...
Previous verdict was "won't fix — live tooltip preferred" after ruling out the only-known fix: writing a static `Name=Claude Usage` and moving live data to `Comment=`. That approach was rejected because `Comment=` is not rendered in the dock hover tooltip (confirmed by screenshot). The obvious alternative — just prepend `"Claude Usage: "` — was overlooked.
...
- Dock hover tooltip: unchanged in character, now prefixed with `Claude Usage: `
