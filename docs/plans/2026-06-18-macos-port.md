...
- **CI** (`.github/workflows/release.yml`): add a `macos-latest` job that runs `build-app.sh`, zips the `.app`, attaches it to the tagged release, and bumps `version` + `sha256` in the tap's cask. Pin actions to Node 24-native majors (`actions/checkout@v6`, `actions/upload-artifact@v7`) per the SRC guide.
